...
# Nova String de Conexão configurada para o Microsoft SQL Server (pyodbc)
...
def receive_data():
    ...
    # 4. Inserção no SQL Server Management Studio
    try:
        # Conexão usando a string ODBC montada
        conn = pyodbc.connect(DB_CONNECTION_STRING)
        cur = conn.cursor()
        
        # Alterado os placeholders de '%s' para '?' padrão do pyodbc/SQL Server
        query_leitura = """
        ...
        conn.commit()
        print("🗄️ Leitura salva com sucesso no SQL Server!")
        
        if estado_formatado in ["ALERTA", "CRÍTICO", "CRITICO"]:
            # Equivalente ao cur.lastrowid no SQL Server para recuperar o ID gerado nesta sessão
            cur.execute("SELECT SCOPE_IDENTITY()")
            id_da_leitura = cur.fetchone()[0]
    ...
    # 5. Inserção no Google Sheets (Inalterado)
    try:
        aba.append_row([
            agora_sheets, 
            data["temperatura"], 
...
if __name__ == "__main__":
    print("🚀 Iniciando API Integrada (SQL Server + Sheets)...")
