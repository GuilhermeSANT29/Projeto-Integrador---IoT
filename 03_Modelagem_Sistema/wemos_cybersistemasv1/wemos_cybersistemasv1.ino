...
// ======================================================
// IP SERVIDOR PYTHON (Atualizado com o seu IP)
// ======================================================
String servidor = "10.106.208.30";   // IP do seu PC com o Flask rodando
...
void enviarDadosParaAPI() {
  ...
  // Verifica validade dos dados do sensor
  ...
  // ✨ JSON CORRIGIDO (Padrão exato da API Python)
  String json = "{"
    "\"temperatura\":" + String(temperatura, 1) + ","
    "\"umidade\":" + String(umidade, 1) + ","
    "\"rotacao\":0,"
    "\"estado\":\"" + determinarStatus(temperatura) + "\""
  "}";
  ...
  if (httpCode > 0) {
    if (httpCode == 201) {
      Serial.println("✅ Dados enviados com sucesso para o banco e sheets!");
    } else {
      Serial.print("⚠️ HTTP não 201: ");
      Serial.println(httpCode);
      Serial.println(http.getString());
    }
  } else {
    Serial.print("❌ Erro HTTP (API Desligada ou IP Errado): ");
    Serial.println(httpCode);
  }
...
void paginaInicial() {
  ...
  html += F("<div class='info-grid'>"
    "<div class='info-item'>"
      ...
      "<div class='info-val blue'>10.106.208.30:5000</div>"
    "</div>"
    "<div class='info-item'>"
      "<div class='info-label'>Database</div>"
      "<div class='info-val blue'>MySQL</div>"
    "</div>"
    "<div class='info-item'>"
  ...
  // FOOTER
  html += F("<footer>CYBERSISTEMAS &copy; 2025 &bull; Wemos D1 Mini + DHT11 + Python API + MySQL + Google Sheets</footer>");
